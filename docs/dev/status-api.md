...
- **`1`**: **正常 (Up)** —— 服务响应正常，一切 OK。 // [!code ++]
- **`0`**: **离线 (Down)** —— 服务已崩溃或无法访问。 // [!code --]
- **`2`**: **待定/异常 (Pending/Warning)** —— 服务虽然在线但可能响应极慢或返回了非预期结果。 // [!code warning]
- **`3`**: **维护中 (Maintenance)** —— 服务处于计划内的维护阶段（通常会跳过通知）。 // [!code focus]
:::
...
async function getUserISP() {
  try {
    // 推荐使用支持 CORS 的 IP-API (免费版通常支持 HTTP 跨域)
...
async function fetchUserISP() {
  try {
    // 方案 1: 使用更稳定的 ip-api.com (支持跨域，中文返回)
    const res = await fetch('http://ip-api.com/json/?lang=zh-CN').catch(() => null)
    if (res) {
      const data = await res.json()
      const isp = data.isp || ''
      if (isp.includes('联通') || isp.includes('Unicom')) userISP.value = '联通'
      else if (isp.includes('电信') || isp.includes('Telecom')) userISP.value = '电信'
      else if (isp.includes('移动') || isp.includes('Mobile')) userISP.value = '移动'
      else userISP.value = '国内线路'
      if (userISP.value !== '识别失败') return
    }

    // 方案 2: 备选方案 (Cloudflare Trace)
    const resCf = await fetch('https://1.1.1.1/cdn-cgi/trace').catch(() => null)
    if (resCf) {
      const text = await resCf.text()
      // 这里需要简单解析，但为了演示先设为识别中
      userISP.value = '识别中...'
    }
  } catch (e) {
    userISP.value = '国内线路'
